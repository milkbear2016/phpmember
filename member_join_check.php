...
//先讀取所有帳號
	...
	$query_AllUser = "SELECT m_username FROM memberdata";
	$AllUser=$db_link->query($query_AllUser);
	if(isset($_GET["finduser"])){
		require_once("connMysql.php");
		//找尋帳號是否已經註冊
		$query_RecFindUser = "SELECT m_username FROM memberdata WHERE m_username='{$_GET["finduser"]}'";
		$RecFindUser=$db_link->query($query_RecFindUser);
		if ($RecFindUser->num_rows>0){
			header("Location: member_join_check.php?errMsg=1&username={$_GET['finduser']}");
	}else{
		header("Location: member_join_check.php?username={$_GET['finduser']}");
	}
}
	//
...
if(isset($_POST["action"])&&($_POST["action"]=="join")){
	...
	$RecFindUser=$db_link->query($query_RecFindUser);
	if ($RecFindUser->num_rows>0){
		header("Location: member_join_check.php?errMsg=1&username={$_POST["m_username"]}");
	}else{
		//若沒有執行新增的動作
		...
		$stmt->close();
		$db_link->close();
		header("Location: member_join_check.php?loginStats=1");
	}
}
?>
<html>
	<head>
		...
		setCookie("newusername",newname,-1000);
		function checkForm(){
					...
					<td class="tdbline"><table width="100%" border="0" cellspacing="0" cellpadding="10">
						<tr valign="top">
							<td class="tdrline"><form action="" method="POST" name="formJoin" id="formJoin" onSubmit="return checkForm();">
								...
								<div class="errDiv">帳號 <?php echo $_GET['username'];?> 可以使用！</div>
								<?php }?>
								<div class="dataDiv">
									<hr size="1" />
									...
									<p><strong>使用帳號</strong>：
										...
										<font color="#FF0000">*</font><br><span class="smalltext">請填入5~12個字元以內的小寫英文字母、數字、以及_ 符號。</span><input type='button' id='checkusername' value="檢查帳號"></p>
										<p><strong>使用密碼</strong>：
											<input name="m_passwd" type="password" class="normalinput" id="m_passwd">
											<font color="#FF0000">*</font><br><span class="smalltext">請填入5~10個字元以內的英文字母、數字、以及各種符號組合，<input type='button' id='showpasswd' value="顯示密碼"><input type='button' id='hidepasswd' value="隱藏密碼"></span></p>
											<p><strong>確認密碼</strong>：
												...
												<script>
													$("#showpasswd").click(function(){
														$("#m_passwd").attr("type","text");
													});
													$("#hidepasswd").click(function(){
														$("#m_passwd").attr("type","password");
													});
													$("#checkusername").click(function(){
														location.href="member_join_check.php?finduser=" +$("#m_username").val();
													});
												</script>
											</body>
										</html>